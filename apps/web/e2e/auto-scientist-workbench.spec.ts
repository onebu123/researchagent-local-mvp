...
test("Auto Scientist workbench exposes tabbed workflow, job controls, and safety labels", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Experiment tree nodes" })).toBeVisible();
  ...
  await expect(page.getByText("No generated-code proposals yet", { exact: false })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Manuscript artifacts" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Reviewer outcome" })).toBeVisible();
});
